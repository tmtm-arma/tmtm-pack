class tmtm_v_rba_base;
class tmtm_v_rba_backplate_base: tmtm_v_rba_base
{
    class ItemInfo;
};

class tmtm_v_type90_glf_olive: tmtm_v_rba_backplate_base
{
    scope=2;
    scopeArsenal=2;

    author="089, erem2k";

    displayName="GLF Type 90 Vest (Olive)";
    picture="\tmtm\pack\main\data\ui\icon_tmtm_wip_ca.paa";

    hiddenSelectionsTextures[]= {
        "\tmtm\pack\faction_glf_sns\characters\vests\data\tmtm_v_rba_glf_olive_co.paa"
    };

    class ItemInfo : ItemInfo
    {
        hiddenSelectionsTextures[]= {
            "\tmtm\pack\faction_glf_sns\characters\vests\data\tmtm_v_rba_glf_olive_co.paa"
        };
    };
};

class tmtm_v_type90_glf_oak: tmtm_v_rba_backplate_base
{
    scope=2;
    scopeArsenal=2;

    author="089, erem2k";

    displayName="GLF Type 90 Vest (Oak)";
    picture="\tmtm\pack\main\data\ui\icon_tmtm_wip_ca.paa";

    hiddenSelectionsTextures[]= {
        "\tmtm\pack\faction_glf_sns\characters\vests\data\tmtm_v_rba_glf_oak_co.paa"
    };

    class ItemInfo : ItemInfo
    {
        hiddenSelectionsTextures[]= {
            "\tmtm\pack\faction_glf_sns\characters\vests\data\tmtm_v_rba_glf_oak_co.paa"
        };
    };
};

class tmtm_v_type90_glf_brushw: tmtm_v_rba_backplate_base
{
    scope=2;
    scopeArsenal=2;

    author="089, erem2k";

    displayName="GLF Type 90 Vest (Brush Woodland)";
    picture="\tmtm\pack\main\data\ui\icon_tmtm_wip_ca.paa";

    hiddenSelectionsTextures[]= {
        "\tmtm\pack\faction_glf_sns\characters\vests\data\tmtm_v_rba_glf_brushw_co.paa"
    };

    class ItemInfo : ItemInfo
    {
        hiddenSelectionsTextures[]= {
            "\tmtm\pack\faction_glf_sns\characters\vests\data\tmtm_v_rba_glf_brushw_co.paa"
        };
    };
};
